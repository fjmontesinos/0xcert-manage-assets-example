...
const btnRevokeAsset = document.getElementById("btnRevokeAsset");
const btnDestroyAsset = document.getElementById("btnDestroyAsset");
...
btnRevokeAsset.addEventListener("click", async () => {
  if (config.assetLedgerSource === "") {
    printWarning(
      "No assetLedgerSource defined. Either deploy a new asset ledger or set asset ledger source in src/config.ts file."
    );
  } else {
    printMessage("Starting asset revoke");
    const mutation = await revokeAsset().catch(e => {
      printError(e);
    });
    if (mutation) {
      printMessage("Asset revoke in progress: " + mutation.id);
      printMessage("This may take a while.");
      await mutation.complete();
      printMessage("Asset revoked.");
    }
  }
});

btnDestroyAsset.addEventListener("click", async () => {
  if (config.assetLedgerSource === "") {
    printWarning(
      "No assetLedgerSource defined. Either deploy a new asset ledger or set asset ledger source in src/config.ts file."
    );
  } else {
    printMessage("Starting asset destroy");
    const mutation = await destroyAsset().catch(e => {
      printError(e);
    });
    if (mutation) {
      printMessage("Asset destroy in progress: " + mutation.id);
      printMessage("This may take a while.");
      await mutation.complete();
      printMessage("Asset destroyed.");
    }
  }
});
